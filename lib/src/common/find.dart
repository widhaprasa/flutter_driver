...
abstract class SerializableFinder {
  ...
  /// Optionally scopes this finder to a specific match index.
  ///
  /// The default value `-1` means no index filtering.
  String get index => '-1';
  ...
  Map<String, String> serialize() {
    final Map<String, String> result = <String, String>{'finderType': finderType};
    if (index != '-1') {
      result['index'] = index;
    }
    return result;
...
class ByTooltipMessage extends SerializableFinder {
  ...
  const ByTooltipMessage(this.text, [this.index = '-1']);
  ...
  static ByTooltipMessage deserialize(Map<String, String> json, {String? path}) {
    ...
    return ByTooltipMessage(text, json['index'] ?? '-1');
  }
}
...
class BySemanticsLabel extends SerializableFinder {
  ...
  const BySemanticsLabel(this.label, [this.index = '-1']);
  ...
  static BySemanticsLabel deserialize(Map<String, String> json, {String? path}) {
    ...
    return BySemanticsLabel(isRegExp ? RegExp(label) : label, json['index'] ?? '-1');
  }
}
...
class ByText extends SerializableFinder {
  ...
  const ByText(this.text, [this.index = '-1']);
  ...
  static ByText deserialize(Map<String, String> json, {String? path}) {
    ...
    return ByText(text, json['index'] ?? '-1');
  }
}
...
class ByValueKey extends SerializableFinder {
  ...
  ByValueKey(this.keyValue, [this.index = '-1'])
  ...
  static ByValueKey deserialize(Map<String, String> json, {String? path}) {
    ...
    return switch (keyValueType) {
      'int' => ByValueKey(int.parse(keyValueString), json['index'] ?? '-1'),
      'String' => ByValueKey(keyValueString, json['index'] ?? '-1'),
...
class ByType extends SerializableFinder {
  ...
  const ByType(this.type, [this.index = '-1']);
  ...
  static ByType deserialize(Map<String, String> json, {String? path}) {
    ...
    return ByType(type, json['index'] ?? '-1');
  }
}
...
/// A Flutter driver command that retrieves an element count using a specified finder.
class GetElementCount extends CommandWithTarget {
  /// Creates a command that counts widgets matching [finder].
  GetElementCount(super.finder, {super.timeout});

  /// Creates a command from a JSON map.
  GetElementCount.deserialize(super.json, super.finderFactory, {super.path}) : super.deserialize();
...
class GetElementCountResult extends Result {
  ...
  /// The number of elements matching the finder.
  final int count;
