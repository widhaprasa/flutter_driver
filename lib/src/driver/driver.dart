...
abstract class FlutterDriver {
  ...
  /// Returns the number of elements matched by [finder].
  Future<int> getElementCount(SerializableFinder finder, {Duration? timeout}) async {
    final Map<String, dynamic> jsonResponse = await sendCommand(
      GetElementCount(finder, timeout: timeout),
    );
    final GetElementCountResult result = GetElementCountResult.fromJson(jsonResponse);
    return result.count;
  }
...
class CommonFinders {
  ...
  /// equal to [text].
  SerializableFinder text(String text, [String index = '-1']) => ByText(text, index);

  /// Finds widgets by [key]. Only [String] and [int] values can be used.
  SerializableFinder byValueKey(dynamic key, [String index = '-1']) => ByValueKey(key, index);

  /// Finds widgets with a tooltip with the given [message].
  SerializableFinder byTooltip(String message, [String index = '-1']) =>
      ByTooltipMessage(message, index);

  /// Finds widgets with the given semantics [label].
  SerializableFinder bySemanticsLabel(Pattern label, [String index = '-1']) =>
      BySemanticsLabel(label, index);

  /// Finds widgets whose class name matches the given string.
  SerializableFinder byType(String type, [String index = '-1']) => ByType(type, index);

  /// Finds the back button on a Material or Cupertino page's scaffold.
  SerializableFinder pageBack([String index = '-1']) => PageBack(index);
