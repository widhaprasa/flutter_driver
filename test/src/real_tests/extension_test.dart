...
void main() {
  ...
  testWidgets('finder index selects specific match', (WidgetTester tester) async {
    await silenceDriverLogger(() async {
      final driverExtension = FlutterDriverExtension((String? arg) async => '', true, true);

      Future<String?> getIndexedText(String index) async {
        final Map<String, String> arguments = GetText(
          ByType('Text', index),
          timeout: const Duration(seconds: 1),
        ).serialize();
        ...
        return GetTextResult.fromJson(response['response'] as Map<String, dynamic>).text;
      }

      await tester.pumpWidget(
        const MaterialApp(
          home: Column(
            children: <Widget>[Text('Hello1'), Text('Hello2'), Text('Hello3')],
          ),
        ),
      );

      expect(await getIndexedText('1'), 'Hello2');
    });
  });
