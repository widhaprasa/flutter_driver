...
void main() {
  ...
  group('VMServiceFlutterDriver', () {
    ...
    group('getText', () {
      ...
      test('sends index when provided', () async {
        fakeClient.responses['get_text'] = makeFakeResponse(<String, dynamic>{'text': 'hello'});
        final String result = await driver.getText(find.text('hello', '1'), timeout: _kTestTimeout);
        expect(result, 'hello');
        expect(fakeClient.commandLog, <String>[
          'ext.flutter.driver {command: get_text, timeout: $_kSerializedTestTimeout, finderType: ByText, index: 1, text: hello}',
        ]);
      });
    });

    group('getElementCount', () {
      test('sends the getElementCount command', () async {
        fakeClient.responses['get_element_count'] = makeFakeResponse(<String, dynamic>{'count': 2});
        final int result = await driver.getElementCount(find.byType('Text'), timeout: _kTestTimeout);
        expect(result, 2);
        expect(fakeClient.commandLog, <String>[
          'ext.flutter.driver {command: get_element_count, timeout: $_kSerializedTestTimeout, finderType: ByType, type: Text}',
        ]);
      });
    });
