...
void main() {
  ...
  group('ByText', () {
    ...
    test('serializes and deserializes with index', () {
      const finder = ByText('hello', '1');
      final ByText roundTrip = ByText.deserialize(finder.serialize());
      expect(roundTrip.text, 'hello');
      expect(roundTrip.index, '1');
      expect(finder.serialize(), <String, String>{
        'finderType': 'ByText',
        'index': '1',
        'text': 'hello',
      });
  ...
  group('GetElementCount', () {
    test('serializes and deserializes', () {
      final command = GetElementCount(ByValueKey('foo'), timeout: const Duration(milliseconds: 7));
      expect(command.serialize(), <String, String>{
      ...
      final GetElementCount deserialized = GetElementCount.deserialize(
        command.serialize(),
        fakeDeserialize,
      );
      expect(deserialized.kind, 'get_element_count');
      expect(deserialized.finder, isA<ByValueKey>());
      expect((deserialized.finder as ByValueKey).keyValue, 'foo');
    });

    test('result serializes and deserializes', () {
      const GetElementCountResult result = GetElementCountResult(3);
      expect(result.toJson(), <String, dynamic>{'count': 3});
      expect(GetElementCountResult.fromJson(<String, dynamic>{'count': 3}).count, 3);
    });
  });
}
